...
beforeEach(() => {
    city = {
        ...
        governmentBuildings: [
            {type:"HOSPITAL",budget:200000,stuffCount:1000,
...
test('city corrected should contains 3 houses', () => {
...
test('city corrected should contains hospital and fire station', () => {
    expect(city.governmentBuildings.length).toBe(2)

    expect(city.governmentBuildings[0].type).toBe("HOSPITAL")
    expect(city.governmentBuildings[0].budget).toBe(200000)
    expect(city.governmentBuildings[0].stuffCount).toBe(1000)
